...
interface ContactMessage {
  ...
  status: 'new' | 'in-progress' | 'resolved' | 'closed' | 'replied';
  tags: string[];
  notes?: string;
  serviceType?: string;
  urgency?: string;
...
// Mesaj yanıtlama için thunk
export const replyToMessage = createAsyncThunk(
  'contact/replyToMessage',
  async ({ id, replyText, replySubject }: { id: string; replyText: string; replySubject: string }, { rejectWithValue }) => {
    try {
      const messageRef = doc(db, 'contact_messages', id);
      await updateDoc(messageRef, {
        status: 'replied',
      ...
      return { id, replyText, replySubject };
    } catch (error) {
      return rejectWithValue(error instanceof Error ? error.message : 'Unknown error');
    }
  }
...
// Dashboard için tüm mesajları getir (filtresiz)
...
const contactSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // Fetch all messages (dashboard and admin page)
    builder
      ...
      .addCase(fetchAllContactMessages.fulfilled, (state, action) => {
        state.isLoading = false;
        // Admin mesajlar sayfası için state'e kaydediyoruz

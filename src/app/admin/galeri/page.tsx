...
const AdminGallery = () => {
  ...
  const { categories: storeCategories, items: storeItems, isLoading } = useAppSelector((state) => state.gallery);
  ...
  // Store'dan gelen verileri formatla
  const categories: GalleryCategory[] = storeCategories.sort((a, b) => (a.order || 0) - (b.order || 0));
  const galleryItems: GalleryItem[] = storeItems.sort((a, b) => (a.order || 0) - (b.order || 0));
  ...
  const handleCategorySubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        ...
        await dispatch(updateGalleryCategory({
        ...
        // Add new category
        await dispatch(addGalleryCategory({
          name: categoryForm.name,
          description: categoryForm.description,
          icon: categoryForm.icon,
          color: categoryForm.color,
  ...
  const handleCategoryDelete = async (categoryId: string) => {
    if (confirm('Bu kategoriyi silmek istediğinizden emin misiniz?')) {
      try {
        await dispatch(deleteGalleryCategory(categoryId)).unwrap();
        fetchData();
      } catch (error) {
  ...
  const handleItemSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let formattedImageUrl = itemForm.imageUrl;
      let formattedThumbnailUrl = itemForm.thumbnailUrl;
      ...
      if (formattedThumbnailUrl && !formattedThumbnailUrl.startsWith('http://') && !formattedThumbnailUrl.startsWith('https://')) {
        formattedThumbnailUrl = `https://${formattedThumbnailUrl}`;
      }
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <CkEditorClient
                      ...
                      onChange={(content) => setItemForm({...itemForm, description: content})}

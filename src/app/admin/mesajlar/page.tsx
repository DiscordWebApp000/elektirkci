...
import { fetchAllContactMessages, updateMessageStatus, replyToMessage } from '@/store/slices/contactSlice';
...
interface Message {
  ...
  status: 'new' | 'read' | 'replied' | 'closed' | 'in-progress' | 'resolved';
...
const AdminMessages = () => {
  ...
  const { messages: storeMessages, isLoading } = useAppSelector((state) => state.contact);
  ...
  const fetchMessages = useCallback(async () => {
    try {
      await dispatch(fetchAllContactMessages()).unwrap();
    } catch (error) {
  ...
  // Store'dan gelen mesajları Message interface'ine uygun hale getir
  const messages: Message[] = storeMessages.map(msg => {
    // Status mapping: store formatından sayfa formatına
    let displayStatus: 'new' | 'read' | 'replied' | 'closed' = 'new';
    if (msg.status === 'in-progress') displayStatus = 'read';
    else if (msg.status === 'resolved') displayStatus = 'replied';
    else if (msg.status === 'closed') displayStatus = 'closed';
  ...
  const handleStatusChange = async (messageId: string, newStatus: string) => {
    try {
      // Store status formatına çevir
      let storeStatus: 'new' | 'in-progress' | 'resolved' | 'closed' = 'new';
      if (newStatus === 'read') storeStatus = 'in-progress';
      else if (newStatus === 'replied') storeStatus = 'resolved';
      else if (newStatus === 'closed') storeStatus = 'closed';
      else storeStatus = 'new';

      await dispatch(updateMessageStatus({
        id: messageId,
        updates: {
          status: storeStatus,
  ...
  const handleReply = (message: Message) => {
    ...
    const serviceTypeText = message.serviceType ? getServiceTypeText(message.serviceType) : 'Mesaj';
    setReplySubject(`Re: ${serviceTypeText} Hakkında`);
    setReplyText('');

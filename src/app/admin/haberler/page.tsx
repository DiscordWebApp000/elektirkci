...
import { fetchAllNews, addNews, updateNews, deleteNews } from '@/store/slices/newsSlice';
...
const AdminNews = () => {
  ...
  const { items: storeNews, isLoading } = useAppSelector((state) => state.news);
  ...
  // Store'dan gelen verileri Haber formatına çevir
  const haberler: Haber[] = storeNews.map(news => ({
    ...news,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let formattedImageUrl = haberForm.imageUrl;
      ...
      const haberData = {
        title: haberForm.title,
        subtitle: haberForm.subtitle,
        description: haberForm.description,
        content: haberForm.content,
        imageUrl: formattedImageUrl,
        tags: haberForm.tags.split(',').map(tag => tag.trim()).filter(tag => tag),
        featured: haberForm.featured,
        isActive: haberForm.isActive,
        slug: haberForm.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, ''),
        order: editingHaber ? editingHaber.order : 0
      };
      ...
      if (editingHaber) {
        ...
        // Add new news
        await dispatch(addNews(haberData)).unwrap();
  ...
  const handleDelete = async (haberId: string) => {
    if (confirm('Bu haberi silmek istediğinizden emin misiniz?')) {
      try {
        await dispatch(deleteNews(haberId)).unwrap();
        fetchHaberler();
      } catch (error) {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <CkEditorClient
                  ...
                  onChange={(content) => setHaberForm({ ...haberForm, content })}

...
import { fetchAllGalleryItems, fetchAllGalleryCategories } from '@/store/slices/gallerySlice';
import { fetchAllNews } from '@/store/slices/newsSlice';
import { fetchAllContactMessages } from '@/store/slices/contactSlice';
...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      // Fetch all collections from store
      const [galleryItemsResult, categoriesResult, newsResult, messagesResult] = await Promise.all([
        dispatch(fetchAllGalleryItems()).unwrap(),
        dispatch(fetchAllGalleryCategories()).unwrap(),
        dispatch(fetchAllNews()).unwrap(),
        dispatch(fetchAllContactMessages()).unwrap()
      ]);

      // Process gallery items
      const galleryItems = galleryItemsResult as GalleryItem[];
      
      // Process categories
      const categories = categoriesResult as GalleryCategory[];
      
      // Process news
      const news = newsResult as NewsItem[];
      
      // Process messages
      const messages = messagesResult as ContactMessage[];
      ...
      recentNews.forEach(item => {
        if (item.createdAt) {
          activities.push({
            ...
            timestamp: convertToTimestamp(item.createdAt),
  ...
  // Helper function to convert string timestamp to Timestamp object for compatibility
  const convertToTimestamp = (timestamp: string | Timestamp): Timestamp => {
    if (typeof timestamp === 'string') {
      // Convert ISO string to Timestamp-like object
      return {
